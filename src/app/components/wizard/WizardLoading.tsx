...
import { createCompleteBoard, type BoardCreationResult } from "../../utils/trelloService";
...
export const WizardLoading = ({ 
  ...
  useEffect(() => {
    ...
    const createBoard = async () => {
      try {
        ...
        // Step 4: Sending invites (already done in createCompleteBoard)
        setCurrentStep(4);
        await new Promise(resolve => setTimeout(resolve, 500));
  ...
  }, [projectName, roles, teamEmails, onComplete, onError]);
  return (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[14px] font-medium ${currentStep > 1 ? 'text-[#10D48E]' : 'text-[#4a5568]'}`}>
                    Created board…
                  </span>
                </div>

                {/* Step 2: Installing lists */}
                <div className="flex items-center gap-3">
                  {currentStep > 2 ? (
                    <div className="w-5 h-5 rounded-full bg-[#10D48E] flex items-center justify-center shrink-0">
                      <Check className="w-3 h-3 text-white stroke-[3]" />
                  ...
                  <span className={`text-[14px] ${currentStep > 2 ? 'text-[#10D48E] font-medium' : currentStep === 2 ? 'text-[#4a5568]' : 'text-[#94a3b8]'}`}>
                    Installing lists…
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[14px] ${currentStep > 3 ? 'text-[#10D48E] font-medium' : currentStep === 3 ? 'text-[#4a5568]' : 'text-[#94a3b8]'}`}>
                    Adding cards…
                  </span>
                </div>

                {/* Step 4: Sending invites */}
                <div className="flex items-center gap-3">
                  {currentStep > 4 ? (
                    <div className="w-5 h-5 rounded-full bg-[#10D48E] flex items-center justify-center shrink-0">
                      <Check className="w-3 h-3 text-white stroke-[3]" />
                  ...
                  <span className={`text-[14px] ${currentStep > 4 ? 'text-[#10D48E] font-medium' : currentStep === 4 ? 'text-[#4a5568]' : 'text-[#94a3b8]'}`}>
                    Sending invites…
                  </span>
                </div>
              </div>
